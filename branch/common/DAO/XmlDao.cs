...
namespace common.DAO
{
    public class XmlDao
    {
        ...
        {
            _xdoc = XElement.Load(xmlConfigPath);
            try
            {
                foreach (var item in nodes)
                {
                    ...
                    _xdoc.Add(newElement);
                 //   _xdoc.Save(xmlConfigPath);
                }
                return _xdoc.Elements().Last();
            }
        ...
        /// <param name="xmlConfigPath">查询的根节点路径</param>
        ...
        {
            XElement xDocument = XElement.Load(xmlPath);
            try
            {
                if (value != null)
                {
        ...
        {
            //若键值对不为0,则需要对属性值进行筛选
            _xdoc = XElement.Load(xmlPath);
            if (property != null)
            {
                IEnumerable<XElement> xElements = _xdoc.Elements(nodePath);
            ...
            {
                XElement xElement = _xdoc.Element(nodePath);
                return xElement;
            }
        }
